...
| :--- | :--- | :--- | :--- | :--- |
| **ADR-001** | **유니버스 수집 정책** | • 전종목(2,500+) 상시 수집<br>• 상한가 단독 수집 | **장전 08:20 일봉 기반 최대 90종목 선별** 후 실시간 WS 수집 | 브로커 단일 연결(LS 200쌍) 내 100% 수용 및 트래픽 안정화. 장중 급등주는 제외 |
| **ADR-002** | **스토리지 파티셔닝** | • 실시간 즉시 Parquet 압축<br>• 단순 기간 경과 삭제 | **3계층 스토리지 & Offload-before-Delete** | 장중 이벤트 루프 블로킹 0% 보장. 원격 바이트 일치 확인 전까지 로컬 원본 영구 보존 |
| **ADR-003** | **시계열 클럭 동기화** | • 로컬 시스템 시계 의존<br>• 비차단 경고 로그 | **NTP 실측 기반 Fail-Closed 클럭 게이트** | 가상머신 클럭 드리프트(+1000ms)로 인한 틱 역전 방어. 2초 초과 시 부트스트랩 거부 |
| **ADR-004** | **주문집행 안전 분기** | • 모의/실전 코드 분리<br>• 실전 플래그 미검증 전송 | **단일 OMS 위 Dual Gateway (Paper vs Armed Live)** | 프로덕션 전환 시 구현 불일치 및 오주문 원천 차단. Live 플래그 누락 시 인스턴스화 거부 |
| **ADR-005** | **아키텍처 불변식** | • 수동 코드 리뷰<br>• 런타임 경로 검증 | **정적 AST 분석 기반 pytest 불변식 강제** | 계층 순환 0건, 상위 역참조 0건, 설정 파일 외부 경로 리터럴 0건을 CI에서 기계적 강제 |
