...
trigger:
  ...
  - on_file_path_regex: "src/.*(engine|data|features|strategy|validation|execution|factor|portfolio).*"
  - on_file_path_glob: ["src/engine/**/*.py", "src/data/**/*.py", "src/features/**/*.py", "src/strategy/**/*.py", "src/factor/**/*.py"]
priority: 10
---
...
- **Escalate When Justified:** If a measured hot path cannot be efficiently handled with standard vectorization (e.g., cross-sectional rank aggregations, high-dimensional rolling metrics), evaluate JIT compilation, native code, out-of-core engines, or parallelism based on the bottleneck shape. Adopt added complexity only when benchmarks demonstrate meaningful gains.
